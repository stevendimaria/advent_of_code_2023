...
class Day17:
    ...
    def move_crucible(self, start, min_mv, max_mv):
        ...
        while h:
            ...
            if not 0 <= r < len(self.blocks) or not 0 <= c < len(self.blocks[0]) or (r, c, d, st) in seen or tot >= ans:
                continue
    ...
    def star1(self):
        self.star1_ans = self.move_crucible((int(self.blocks[0][0]) * -1, 0, 0, 0, 'R'), 1, 3)
        print(f"Day 17, Star 1 Answer: {self.star1_ans}")
        return self.star1_ans

    def star2(self):
        self.star2_ans = self.move_crucible((int(self.blocks[0][0]) * -1, 0, 0, 0, 'R'), 4, 10)
